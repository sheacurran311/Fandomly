...
import { CREATOR_FIELD_INFO, CREATOR_VERIFICATION_REQUIREMENTS } from "@shared/creatorVerificationSchema";
import type { CreatorVerificationData } from "@shared/creatorVerificationSchema";

interface CreatorVerificationProgressProps {
  creator: any; // Full creator object from database
...
export function CreatorVerificationProgress({
  ...
  const typeSpecificFields = CREATOR_VERIFICATION_REQUIREMENTS[
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`p-3 rounded-full ${profileComplete ? 'bg-green-500/10' : 'bg-yellow-500/10'}`}>
              {profileComplete ? (
      ...
      <CardContent className="space-y-6">
        ...
        {profileComplete ? (
          <Alert className="bg-green-500/10 border-green-500/20">
            ...
            <AlertDescription className="text-green-400">
              Congratulations! Your profile is verified. You now have access to all creator features and your profile displays a verified badge.
              {verifiedAt && (
          ...
          <Alert className="bg-yellow-500/10 border-yellow-500/20">
            ...
            <AlertDescription className="text-yellow-400">
              Complete the required fields below to verify your profile and unlock all creator features.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <h4 className="font-semibold text-sm flex items-center gap-2">
              {creatorType === 'athlete' ? 'Athletic' : creatorType === 'musician' ? 'Music' : 'Content Creator'} Information
              <Badge variant="secondary" className="text-xs">
                {typeSpecificFields.filter(f => isFieldComplete(f)).length}/{typeSpecificFields.length}
              </Badge>
            </h4>
            <div className="space-y-2">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-medium text-sm">Connect at least one social media account</p>
                  <p className="text-xs text-gray-400">
                    {isFieldComplete('socialMedia')
                      ? `Connected: ${socialFields.filter(s => requiredFieldsFilled.includes(s)).map(s => CREATOR_FIELD_INFO[s]?.label).join(', ')}`
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {benefits.map((benefit, index) => (
              <div
                ...
                `}
              >
                <benefit.icon className={`h-5 w-5 ${benefit.locked ? 'text-gray-400' : 'text-green-500'}`} />
