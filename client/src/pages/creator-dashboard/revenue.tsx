  ...
  const { data: revenueData, isLoading: revenueLoading } = useQuery<RevenueData>({
    ...
    queryFn: async (): Promise<RevenueData> => {
      ...
      try {
        ...
        const historicalFanCount = memberships.filter(
          (m: Record<string, unknown>) => new Date(m.joinedAt as string) < thirtyDaysAgo
        ).length;
    ...
    ? [
        {
          ...
          percentage:
            revenueData.totalRevenue > 0
              ? (revenueData.subscriptionRevenue / revenueData.totalRevenue) * 100
              : 0,
        ...
        },
        {
          ...
          percentage:
            revenueData.totalRevenue > 0
              ? (revenueData.campaignRevenue / revenueData.totalRevenue) * 100
              : 0,
    ...
    queryFn: async (): Promise<Transaction[]> => {
      ...
      try {
        ...
        for (const program of programs.slice(0, 2)) {
          // Limit to first 2 programs for performance
          try {
            ...
            transactions.slice(0, 3).forEach((tx: Record<string, unknown>) => {
              allTransactions.push({
                id: tx.id as string,
                type: (tx.campaignName as string) || 'Points Awarded',
                amount: (tx.pointsAwarded as number) || 0,
                date: new Date(tx.timestamp as string).toLocaleDateString(),
                fan: (tx.fanId as string) || 'Fan',
                status: 'completed',
              });
            });
          } catch (error) {
              ...
              <Card className="bg-white/5 backdrop-blur-lg border-white/10">
                <CardContent className="p-6">
                  ...
                  <div className="mt-2">
                    <Button
                      size="sm"
                      className="bg-brand-primary hover:bg-brand-primary/80 text-xs"
                    >
                      Withdraw
                    </Button>
                  </div>
              ...
              <div className="space-y-3">
                ...
                {(() => {
                  ...
                  return monthLabels.map(({ label, year }, index) => {
                    const decayFactor = 1 - (5 - index) * 0.1;
                    const value = Math.floor(baseValue * Math.max(decayFactor, 0.5));
                  ...
                  <span className="text-white font-medium">
                    ...
                    {(() => {
                      ...
                      const values = Array.from({ length: 6 }, (_, i) =>
                        Math.floor(base * Math.max(1 - (5 - i) * 0.1, 0.5))
                      );
                      const avg =
                        values.length > 0
