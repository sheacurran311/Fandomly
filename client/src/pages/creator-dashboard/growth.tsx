...
export default function CreatorGrowth() {
  ...
  const { data: socialGrowth, isLoading: socialGrowthLoading } = useGrowthAnalytics(
    platformsParam,
    dateRange
  );
    ...
    queryFn: async (): Promise<GrowthMetrics> => {
      ...
      try {
        ...
        const membershipsResponse = await apiRequest(
          'GET',
          `/api/tenant-memberships/${creator.tenantId}`
        );
        const memberships = await membershipsResponse.json();

        const currentFans = memberships.length;
        const activeFans = memberships.filter(
          (m: Membership) => (m.memberData?.points || 0) > 0
        ).length;
        const totalPoints = memberships.reduce(
          (sum: number, m: Membership) => sum + (m.memberData?.points || 0),
          0
        );
        ...
        const previousMemberships = memberships.filter(
          (m: Membership) => new Date(m.joinedAt) < thirtyDaysAgo
        );
        const previousFans = previousMemberships.length;
        const previousActiveFans = previousMemberships.filter(
          (m: Membership) => (m.memberData?.points || 0) > 0
        ).length;
        const previousTotalPoints = previousMemberships.reduce(
          (sum: number, m: Membership) => sum + (m.memberData?.points || 0),
          0
        );
        const previousAveragePoints = previousFans > 0 ? previousTotalPoints / previousFans : 0;
        const previousEngagementRate =
          previousFans > 0 ? (previousActiveFans / previousFans) * 100 : 0;

        // Calculate percentage changes
        const fanGrowthChange =
          previousFans > 0 ? ((currentFans - previousFans) / previousFans) * 100 : 0;
        const engagementChange =
          previousEngagementRate > 0
            ? ((engagementRate - previousEngagementRate) / previousEngagementRate) * 100
            : 0;
        const averagePointsChange =
          previousAveragePoints > 0
            ? ((averagePoints - previousAveragePoints) / previousAveragePoints) * 100
            : 0;
        ...
        for (const program of programs) {
          try {
            const transactionsResponse = await apiRequest(
              'GET',
              `/api/point-transactions/program/${program.id}`
            );
            const transactions = await transactionsResponse.json();

            // Current period (last 30 days)
            const recentTransactions = transactions.filter(
              (tx: Transaction) => new Date(tx.timestamp) > thirtyDaysAgo
            );
            const recentUniqueFans = new Set(recentTransactions.map((tx: Transaction) => tx.fanId));
            currentCampaignParticipants += recentUniqueFans.size;

            // Previous period (30-60 days ago)
            const sixtyDaysAgo = new Date();
            sixtyDaysAgo.setDate(sixtyDaysAgo.getDate() - 60);
            const previousTransactions = transactions.filter((tx: Transaction) => {
              const txDate = new Date(tx.timestamp);
              return txDate > sixtyDaysAgo && txDate <= thirtyDaysAgo;
            });
            const previousUniqueFans = new Set(
              previousTransactions.map((tx: Transaction) => tx.fanId)
            );
        ...
        const currentParticipationRate =
          currentFans > 0 ? (currentCampaignParticipants / currentFans) * 100 : 0;
        const previousParticipationRate =
          previousFans > 0 ? (previousCampaignParticipants / previousFans) * 100 : 0;
        const participationChange =
          previousParticipationRate > 0
            ? ((currentParticipationRate - previousParticipationRate) / previousParticipationRate) *
              100
            : 0;
  ...
  const growthGoals: GrowthGoal[] = growthMetrics
    ? (() => {
        const fanTarget =
          Math.ceil(
            ((growthMetrics?.fanGrowth?.current || 0) + DEFAULT_GROWTH_GOALS.fanGrowthIncrement) /
              100
          ) * 100;
  ...
  const _getChangeColor = (change: number) => {
    return change > 0 ? 'text-green-400' : 'text-red-400';
  };

  const _getChangeIcon = (change: number) => {
    ...
    <DashboardLayout userType="creator">
      <div className="p-6">
            ...
            <>
                ...
                <CardContent className="p-6">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-white">
                      {(growthMetrics?.fanGrowth?.current || 0).toLocaleString()}
                    </p>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      {(
                        ...
                        (growthMetrics?.fanGrowth?.previous || 0)
                      ).toLocaleString()}{' '}
                      this month
                    </p>
                  </div>
              ...
              <Card className="bg-white/5 backdrop-blur-lg border-white/10">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge className="bg-green-500/20 text-green-400">
                      +{growthMetrics?.engagementRate?.change || 0}%
                    </Badge>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-white">
                      {(growthMetrics?.engagementRate?.current || 0).toFixed(1)}%
                    </p>
                    <p className="text-sm text-gray-400">Engagement Rate</p>
                    <p className="text-xs text-gray-500 mt-1">
                      +
                      {(
                        (growthMetrics?.engagementRate?.current || 0) -
                        (growthMetrics?.engagementRate?.previous || 0)
                      ).toFixed(1)}
                      % vs last month
                    </p>
                  </div>
              ...
              <Card className="bg-white/5 backdrop-blur-lg border-white/10">
                <CardContent className="p-6">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-white">
                      {(growthMetrics?.campaignParticipation?.current || 0).toFixed(1)}%
                    </p>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      {(
                        ...
                        (growthMetrics?.campaignParticipation?.previous || 0)
                      ).toFixed(1)}
                      % improvement
                    </p>
                  </div>
              ...
              <Card className="bg-white/5 backdrop-blur-lg border-white/10">
                <CardContent className="p-6">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-white">
                      {(growthMetrics?.averagePointsPerFan?.current || 0).toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-400">Avg Points/Fan</p>
                    <p className="text-xs text-gray-500 mt-1">
                      +
                      {(growthMetrics?.averagePointsPerFan?.current || 0) -
                        (growthMetrics?.averagePointsPerFan?.previous || 0)}{' '}
                      pts increase
                    </p>
                  </div>
                </CardContent>
        ...
        <div className="mb-8">
          ...
          <PlatformBreakdown platforms={overview?.platforms || []} isLoading={overviewLoading} />
        </div>
