...
export default function CreatorSocial() {
  ...
  const youtubeSubscribers =
    youtubeUserInfo?.followersCount || youtubeUserInfo?.followers_count || 0;
  ...
  const kickFollowers = kickUserInfo?.followersCount || kickUserInfo?.followers_count || 0;

  // Patreon connection via standardized hook
  const {
    isConnected: patreonConnected,
    isConnecting: patreonConnecting,
  ...
  const getTotalFollowers = () => {
    ...
    // Facebook page followers, Discord, and Patreon don't have follower counts
    return total;
  };
  ...
  const socialAccounts = [
    ...
    },
    {
      ...
      followers: twitterConnected
        ? `${formatFollowers(twitterFollowers)} Followers`
        : '0 Followers',
    ...
    },
    {
      ...
      followers: spotifyConnected
        ? `${formatFollowers(spotifyFollowers)} Followers`
        : '0 Followers',
    ...
    // Facebook managed by state above
    {
      ...
      icon: Facebook,
      handle:
        facebookConnected && activePage
          ? activePage.name
          : facebookConnected
            ? 'No active page'
            : '@your-page',
      followers:
        facebookConnected && activePage?.followers_count
          ? `${activePage.followers_count.toLocaleString()}`
          : '0',
      engagement:
        facebookConnected && activePage
          ? 'Active'
    ...
    },
    {
      platform: 'Kick',
      icon: SiKick,
      handle: kickConnected && kickDisplayName ? kickDisplayName : 'Not connected',
      followers: kickConnected ? `${formatFollowers(kickFollowers)} Followers` : '0 Followers',
      engagement: kickConnected ? 'Active' : '—',
    ...
    },
    {
      platform: 'Patreon',
      icon: FaPatreon,
      handle: patreonConnected && patreonDisplayName ? patreonDisplayName : 'Not connected',
      followers: '—',
      engagement: patreonConnected ? 'Active' : '—',
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-4 flex-1 min-w-0">
                      <div
                        className={`w-12 h-12 flex-shrink-0 ${account.bgColor} rounded-full flex items-center justify-center`}
                      >
                        ...
                        <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                          ...
                          {account.connected ? (
                            ...
                            <>
                              <Badge
                                variant="outline"
                                className="border-gray-500/30 text-gray-400 text-xs flex-shrink-0"
                              >
                          ...
                          account.connected ? (
                            <>
                              <Button
                                ...
                                data-testid="button-disconnect-kick-social"
                              >
                                <Unlink className="h-4 w-4 mr-1" />
                                Disconnect
                              </Button>
                            ...
                            <Button
                              ...
                              data-testid="button-connect-kick-social"
                            >
                              {kickConnecting ? 'Connecting...' : 'Connect'}
                            </Button>
                          )
                        ) : account.platform === 'Patreon' ? (
                          account.connected ? (
                            <>
                              <Button
                                variant="outline"
                                ...
                                data-testid="button-disconnect-patreon-social"
                              >
                                <Unlink className="h-4 w-4 mr-1" />
                                Disconnect
                              </Button>
        ...
        <DialogContent className="sm:max-w-md bg-brand-dark-bg border-white/10">
          ...
          <div className="space-y-4">
            <div className="text-sm text-gray-400 mb-4">
              Select which Facebook page to use for your campaigns. Only one page can be active at a
              time.
            </div>
