...
export default function FanTasksPage() {
  ...
  const [viewType, setViewType] = useState<ViewType>(() => {
    const saved = localStorage.getItem('fanTasksView') as ViewType;
    return saved || 'table';
  });

  // Load view preference from localStorage using lazy initializer in useState above
      ...
      tasks.filter((task: Task) => {
        ...
        if (filterType !== 'all') {
          ...
          if (filterType === 'available') {
            ...
            const isCompletedButAvailable =
              completion?.status === 'completed' &&
              (completion as TaskCompletion & { isAvailableAgain?: boolean })?.isAvailableAgain;
  ...
  const taskTypeBreakdown = tasks.reduce((acc: Record<string, number>, task: Task) => {
      ...
      <div className="container max-w-7xl mx-auto py-8 px-4">
        ...
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-400">
            Complete tasks from creators you&apos;re enrolled with to earn creator-specific rewards
          </p>
        </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className={
                    viewType === 'table' ? 'bg-brand-primary hover:bg-brand-primary/80' : ''
                  }
                >
                ...
                <Button
                  ...
                  className={
                    viewType === 'cards' ? 'bg-brand-primary hover:bg-brand-primary/80' : ''
                  }
                >
        ...
        {isLoading ? (
          ...
          <FanTasksTable tasks={filteredTasks as Task[]} completionMap={completionMap} />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTasks.map(
              (
                task: Task & { programName?: string; programImage?: string; creatorName?: string }
              ) => {
                const completion = completionMap.get(task.id);
                return (
                  <FanTaskCard
                    ...
                    progress={{
                      ...
                      currentStreak:
                        (completion?.completionData as { currentStreak?: number })?.currentStreak ??
                        0,
                    }}
