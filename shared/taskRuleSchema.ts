...
export const taskRuleSchema = z.object({
  ...
  hideFromUI: z.boolean().default(false).describe('Hidden background task, triggered via API only'),
...
export const referralSettingsSchema = z.object({
  ...
  referrerPoints: z.number().min(0).optional().describe('Points awarded to the person who refers'),
...
export const checkInSettingsSchema = z.object({
  pointsPerCheckIn: z.number().min(1).describe('Base points awarded for each check-in'),

  // Streak system
  enableStreak: z.boolean().default(false).describe('Enable consecutive check-in streak bonuses'),

  rewardOnlyStreakCompletions: z
    .boolean()
    .default(false)
  ...
  streakMilestones: z
    .array(
      z.object({
        consecutiveDays: z.number().min(2).describe('Number of consecutive days required'),
        bonusPoints: z.number().min(1).describe('Bonus points awarded for reaching this milestone'),
      })
    )
    .optional(),
...
export const followerMilestoneSettingsSchema = z.object({
  ...
  tiers: z
    .array(
      z.object({
        followerCount: z.number().min(1).describe('Follower count required for this tier'),
        points: z.number().min(1).describe('Points awarded for reaching this tier'),
      })
    )
    .optional(),
});
...
export const connectAccountSettingsSchema = z.object({
  ...
  supportedWallets: z.array(z.enum(['evm', 'solana', 'bitcoin', 'sui', 'ton'])).optional(),
});
...
export const socialActionSettingsSchema = z.object({
  ...
  handle: z.string().optional().describe('Twitter/Instagram/TikTok handle to follow'),

  url: z.string().url().optional().describe('Direct URL to post/video/page to interact with'),

  pageId: z.string().optional().describe('Facebook page ID'),

  channelUrl: z.string().url().optional().describe('YouTube channel URL'),

  artistId: z.string().optional().describe('Spotify artist ID'),
  ...
  hashtags: z.array(z.string()).optional().describe('Required hashtags for post tasks'),
...
export const codeEntrySettingsSchema = z.object({
  code: z.string().min(1).describe('The code that fans must enter'),

  caseSensitive: z.boolean().default(false).describe('Whether the code is case-sensitive'),
...
export const TASK_CATEGORIES = {
  user_onboarding: {
    ...
    tasks: ['complete_profile', 'connect_email', 'connect_wallet', 'refer_friend', 'check_in'],
  },
  ...
  custom: {
    ...
    tasks: ['enter_code', 'click_link', 'answer_quiz', 'answer_poll', 'submit_text'],
  },
} as const;
...
export function validateRewardConfig(data: Partial<TaskRule>): { valid: boolean; error?: string } {
  ...
  if (data.rewardType === 'multiplier') {
    if (!data.multiplierValue || data.multiplierValue <= 1) {
      return {
        valid: false,
        error: "Multiplier value must be greater than 1 when reward type is 'multiplier'",
      };
    }
  }
