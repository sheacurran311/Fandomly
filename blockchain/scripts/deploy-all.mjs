/**
 * Deploy ALL Fandomly contracts to Avalanche Fuji C-Chain (testnet)
 *
...
async function main() {
  ...
  console.log('==============================================');
  console.log('  Avalanche Fuji — Full Contract Deployment');
  ...
  if (balance === 0n) {
    console.error('  ERROR: Deployer has no AVAX. Fund via https://faucet.avax.network/');
    process.exit(1);
  }
  ...
  // Update ONLY the CONTRACTS and DEPLOYER_ADDRESS in blockchain-config.ts
  // IMPORTANT: Preserves ABIs and other exports that are manually maintained
  ...
  // Replace DEPLOYER_ADDRESS
  config = config.replace(
    /export const DEPLOYER_ADDRESS = '0x[a-fA-F0-9]+' as const;/,
    `export const DEPLOYER_ADDRESS = '${account.address}' as const;`
  );

  writeFileSync(configPath, config);
  console.log(`  Config updated: shared/blockchain-config.ts (addresses only — ABIs preserved)`);
}
