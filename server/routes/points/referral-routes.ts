...
import { db } from '../../db';
import { creators, creatorReferrals, fanReferrals, creatorTaskReferrals } from '@shared/schema';
...
export function registerReferralRoutes(app: Express) {
  ...
  app.get('/api/referrals/creator', authenticateUser, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      console.error('[Referral] Error fetching creator referral:', error);
   ...
   */
  app.post('/api/referrals/creator/track-click', publicEndpointLimiter, async (req, res) => {
    try {
      const { code } = req.body;
   ...
   */
  app.post(
    '/api/referrals/creator/complete',
    authenticateUser,
  ...
  app.get('/api/referrals/fan', authenticateUser, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      console.error('[Referral] Error fetching fan referral:', error);
      res.status(500).json({ error: error.message });
    }
  });
   ...
   */
  app.post('/api/referrals/fan/track-click', publicEndpointLimiter, async (req, res) => {
    try {
      const { code } = req.body;
   ...
   */
  app.post('/api/referrals/task/track-click', publicEndpointLimiter, async (req, res) => {
    try {
      const { code } = req.body;
  ...
  app.get('/api/referrals/task/stats', authenticateUser, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const stats = await creatorTaskReferralService.getFanTaskReferralStats(userId, creatorId);
