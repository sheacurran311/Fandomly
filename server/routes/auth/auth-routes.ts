...
export function registerAuthRoutes(app: Express) {
  ...
  app.post('/api/auth/social/callback', async (req: Request, res: Response) => {
    try {
      const { provider, access_token, platform_user_id, email, username, display_name, profile_data } = req.body;
      ...
      if (existingConnection) {
        ...
        if (email) {
          ...
          if (user) {
            ...
            return res.status(200).json({
              success: false,
              linkRequired: true,
              existingProviders: user.primaryAuthProvider ? [user.primaryAuthProvider] : ['unknown'],
              pendingLinkId,
        ...
        // Create new user
        isNewUser = true;
        const generatedUsername = generateUsername(display_name || username || email?.split('@')[0] || 'user');
   ...
   */
  app.get('/api/auth/linked-accounts', authenticateUser, async (req: AuthenticatedRequest, res: Response) => {
    try {
      if (!req.user?.id) {
   ...
   */
  app.post('/api/auth/initiate-link', authenticateUser, async (req: AuthenticatedRequest, res: Response) => {
    try {
      if (!req.user?.id) {
        return res.status(401).json({ error: 'Authentication required' });
      }
      ...
      const linkToken = Buffer.from(JSON.stringify({
        userId: req.user.id,
        provider,
  ...
  app.post('/api/auth/complete-link', async (req: Request, res: Response) => {
    try {
      const { link_token, access_token, platform_user_id, username, display_name, profile_data } = req.body;

      if (!link_token || !access_token || !platform_user_id) {
        return res.status(400).json({ error: 'link_token, access_token, and platform_user_id are required' });
      }
