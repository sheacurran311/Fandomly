    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      `);
        ...
        const recentCompletions = await db
          ...
          .where(and(eq(tasks.creatorId, creator.id), eq(taskCompletions.status, 'completed')))
    ...
    try {
      ...
      const joinedPrograms = await db
        .select({
          ...
          pointsBalance: fanPrograms.currentPoints,
          tier: fanPrograms.currentTier,
      ...
      res.json({
        stats: {
          points: {
            ...
            total:
              (parseInt(stats.total_platform_points) || 0) +
              (parseInt(stats.total_creator_points) || 0),
          },
          tasks: {
            ...
            total:
              (parseInt(stats.platform_tasks_completed) || 0) +
              (parseInt(stats.creator_tasks_completed) || 0),
          },
          engagement: {
