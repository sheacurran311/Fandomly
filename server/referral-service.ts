...
import { 
  ...
  tasks,
  campaigns,
  type InsertCreatorReferral,
  type InsertFanReferral,
  type InsertCreatorTaskReferral,
  type CreatorReferral,
  type FanReferral,
  type CreatorTaskReferral
} from "@shared/schema";
import { eq, and, sql, desc } from "drizzle-orm";
import { fandomlyPointsService, creatorPointsService } from "./points-service";
 ...
 */
function generateReferralUrl(type: 'creator' | 'fan' | 'task' | 'campaign', params: {
  code: string;
  creatorUrl?: string;
  taskId?: string;
  campaignId?: string;
...
export class CreatorReferralService {
  ...
  async getOrCreateCreatorReferral(creatorId: string): Promise<CreatorReferral> {
    ...
    const [referral] = await db.insert(creatorReferrals).values({
...
export class FanReferralService {
  ...
  async getOrCreateFanReferral(fanId: string): Promise<FanReferral> {
    ...
    const [referral] = await db.insert(fanReferrals).values({
...
export class CreatorTaskReferralService {
  ...
  async createTaskReferral(
    ...
    const [referral] = await db.insert(creatorTaskReferrals).values({
      creatorId,
      taskId,
      referringFanId: fanId,
      referralCode: code,
   ...
   */
  async trackClick(code: string): Promise<void> {
    await db.update(creatorTaskReferrals)
      .set({ 
