...
function verifyFacebookSignature(body: any, signature: string, appSecret: string): boolean {
  if (!signature || !appSecret) {
    console.log('[Facebook Webhooks] Signature verification skipped - missing signature or app secret');
    return false;
  }
  ...
  for (const change of entry.changes || []) {
    ...
    switch (field) {
      case 'feed':
        ...
        // TODO: Match to pending tasks and verify
        break;
      ...
      case 'reactions':
        ...
        if (value.verb === 'add' && value.sender_id) {
          await matchAndVerifyTask(
            ...
            'facebook',
            'like',
            { postId: value.post_id, reactionType: value.reaction_type }
          );
        }
        break;
  ...
  for (const change of entry.changes || []) {
    ...
    switch (field) {
      case 'photos':
        ...
        // TODO: Verify photo-related tasks
        break;
      ...
      case 'videos':
        ...
        // TODO: Verify video-related tasks
        break;
      ...
      case 'feed':
        ...
        // TODO: Verify feed activity tasks
        break;
...
export function registerFacebookWebhooks(app: Express) {
  ...
  app.post('/webhooks/facebook/pages', async (req: Request, res: Response) => {
    ...
    if (signature && appSecret) {
      ...
      console.warn('[Facebook Pages Webhooks] ⚠️ Signature verification skipped (development mode)');
    }
  ...
  app.post('/webhooks/facebook/users', async (req: Request, res: Response) => {
    ...
    if (signature && appSecret) {
      ...
      console.warn('[Facebook Users Webhooks] ⚠️ Signature verification skipped (development mode)');
    }
